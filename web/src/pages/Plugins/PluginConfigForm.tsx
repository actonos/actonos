...
export function PluginConfigForm({ plugin, onSaved, onCancel }: PluginConfigFormProps) {
  ...
  const schemaProperties = (configSchema.properties || {}) as Record<string, any>;
  ...
  const groupedProperties = useMemo(() => {
    const groups: Record<string, Array<{ key: string; prop: any }>> = {};
    const defaultGroup = 'General Settings';

    Object.entries(schemaProperties).forEach(([key, prop]) => {
      const groupName = prop['x-ui-group'] || defaultGroup;
      if (!groups[groupName]) {
        groups[groupName] = [];
      }
    ...
    Object.keys(groups).forEach((g) => {
      groups[g].sort((a, b) => (a.prop['x-order'] ?? 0) - (b.prop['x-order'] ?? 0));
    });
    ...
    try {
      ...
      Object.entries(schemaProperties).forEach(([k, prop]) => {
        if (prop.type === 'array' && Array.isArray(formData[k])) {
          const itemProps = (prop.items?.properties || {}) as Record<string, any>;
          (formData[k] as Array<Record<string, any>>).forEach((item, idx) => {
            Object.entries(itemProps).forEach(([subKey, subProp]) => {
              if ((subProp['x-secret'] || subProp['x-ui-widget'] === 'password') && item[subKey] && typeof item[subKey] === 'string') {
                const subVal = item[subKey].trim();
                if (subVal) {
                  const accountId = item.account_id || (idx === 0 ? 'default' : `item_${idx}`);
  ...
  const renderArrayField = (key: string, prop: any) => {
    const items = (formData[key] || []) as Array<Record<string, any>>;
    const itemSchema = prop.items || {};
    const itemProps = (itemSchema.properties || {}) as Record<string, any>;
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            {prop.description && (
              <p className="text-caption text-slate">{prop.description}</p>
            )}
          </div>
          <Button
            type="button"
        ...
        {items.length === 0 ? (
          <div className="p-4 rounded-2xl border border-dashed border-onyx/15 text-center text-caption text-slate bg-canvas">
              ...
              >
                <div className="flex items-center justify-between pb-2 border-b border-onyx/10">
                  <span className="text-caption font-bold text-deep-ink flex items-center gap-2">
                    ...
                    {item.display_name || item.account_id || `Item #${idx + 1}`}
                  </span>
                  <button
                    type="button"
                      ...
                      <div key={subKey}>
                        ...
                        <div className="relative">
                          <input
                            ...
                            placeholder={subProp['x-ui-placeholder'] || ''}
                            value={subVal}
                            onChange={(e) => handleItemChange(idx, subKey, e.target.value)}
                          ...
                          {isSecret && (
                            <button
                              type="button"
                              onClick={() => toggleShowSecret(`${key}_${idx}_${subKey}`)}
  ...
  const renderSingleField = (key: string, prop: any) => {
      ...
      <div key={key}>
        ...
        <div className="relative">
          <input
            ...
            min={prop.minimum}
            max={prop.maximum}
            placeholder={prop['x-ui-placeholder'] || (isVaultConfigured ? '•••••••• (Configured in Vault)' : '')}
            value={val}
      ...
      {Object.keys(groupedProperties).length > 0 ? (
        <div className="space-y-6">
          {Object.entries(groupedProperties).map(([groupName, items]) => (
            <div
              key={groupName}
              className="p-5 rounded-2xl bg-soft-meadow border border-onyx/10 space-y-4"
            >
              <div className="flex items-center gap-2 pb-2.5 border-b border-onyx/10">
                <Sliders className="w-4 h-4 text-slate" />
                <h4 className="font-serif text-body font-bold text-deep-ink">
                  {groupName}
                </h4>
              </div>

              <div className="space-y-4">
                {items.map(({ key, prop }) => renderSingleField(key, prop))}
              </div>
            </div>
          ))}
        ...
        /* No configuration required fallback */
        <div className="py-10 text-center text-slate font-sans bg-soft-meadow rounded-2xl border border-onyx/10 p-6">
          <Settings2 className="w-8 h-8 opacity-40 mx-auto mb-2 text-slate" />
          <p className="text-body font-semibold text-deep-ink">
            {t('config.noConfigNeeded', 'No configuration schema defined for this plugin.')}
          </p>
          <p className="text-caption text-slate mt-1 max-w-md mx-auto">
            {t('config.noConfigNeededDesc', 'This plugin operates statelessly without external credentials or configurable parameters in manifest.config_schema.')}
          </p>
        </div>
      )}
    </form>
