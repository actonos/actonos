...
func (s *ConstraintsSection) Render() string {
	...
	if s.AgentSlug != "" {
		...
		sb.WriteString("  <rule id=\"user_workspace_mandate\">USER WORKSPACE MANDATE: Whenever the user asks to save, create, read, search, or delete files in their workspace (e.g. 'save to workspace', 'create a file for me', 'read documents in workspace', 'save plan/document'), you MUST ALWAYS invoke `native_workspace_*` tools (`native_workspace_write`, `native_workspace_read`, `native_workspace_search`, `native_workspace_delete`). Files created with `native_file_write` are private internal scratchpads and are NOT visible to the user on their Workspace page. For binary files (PDF, images, archives, docs), build them in your scratchpad and publish with `native_workspace_write` using `from_path`. To process an uploaded PDF or other binary, native_workspace_search for exec_path then open it from native_exec via $ACTONOS_USER_WORKSPACE/exec_path or user-workspace/exec_path — never pass the UUID blob path to python.</rule>\n")
	}
